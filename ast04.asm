...
%macro converttobase 2
 
        mov rax, 0           ; running total
        mov rcx, %1         ; pointer to string
    ...
    %%check_upper:
        ;uppercase
        cmp bl, 'A'   ; if at least 'A'
        jb %%check_lower   ;if below skip
        cmp bl, 'K'   ;  if higher then k
        jle %%digit_uppercase  
    ...
    %%digit_numeric:
        movzx rbx, bl ; set upper rbx to 0 
        sub rbx, '0'        ; turn into real numbers
        add rax, rbx
        jmp %%next_char
...
%macro strLength 2
    ...
    mov rcx, 0        ; counter = 0
    mov rsi, %1       ; pointer to start of string

    %%loop:
    mov al, [rsi + rcx]  ; load value at index rcx
    cmp al, NULL         ; check if null
    je %%done
...
_start:
    ...
    converttobase number, convertednum  ;convert number to decimel

     
    converttobase number2, convertednum2 ;convert number2 to decimal


    strLength number, length ; find length of string
